...
const {
    ...
    City_User,
    Team_User,
    Team_Client_City_Acces,
    Team_Client_Column_Acces,
    Team_Client_Product_Acces,
    Team_Client_Page_Acces,
    Product,
    Client_Page,
    Setting_User,
    Stock,
...
class ClientServices {
    ...
    static async #findClientById(id) {
        var clientFound = await User.findOne({
            where: { id },
            include: [{ model: Subscription, include: Pack }, Setting_Admin],
        });
    ...
    static async AddCityByUploadFile({ id_user, cities }) {
        async function getShippingCompaniesByName(name) {
            const companies = await Shipping_Companie.findOne({
                where: { name: name },
            });
    ...
    static async PatchColumnOfOrder({
        id,
        id_user,
        name,
        alias,
        ...
        isExist.alias = alias.replace(/ /g, "_") ?? isExist.alias;
    ...
    static async GetColumnOfOrder({ id, id_user }) {
        if (!id) {
            try {
                ...
                ColumnOfOrder = ColumnOfOrder.map(x => x.get({ plain: true }))

                ColumnOfOrder.forEach(
                    (column) => {
                        let columnAlias = column?.alias || "";
                        if (columnAlias) {
                            const formattedValue = columnAlias.replace(/_/g, " ");
                            column.alias = formattedValue;
                        }
                    }
                );
            ...
            try {
                var isExist = await this.#findColumnOfOrder(id, id_user);
                if (!isExist)
                    return { code: 404, message: "This ressource doesn't exist" };
    ...
    static async AddCity({
        ...
        if (isExistCity) {
            if (!isExistCity.isDeleted) {
                if (!isExistCity.isFromSheet) {
                    return { code: 400, message: "This city already exist" };
                } else {
                    return { code: 400, message: "This city already imported through Sheet" };
                }
            }
    ...
    static async RemoveCity({ id, id_user }) {
        ...
        try {
            ...
            if (error.name === "SequelizeForeignKeyConstraintError") {
                return {
                    code: 400,
                    message: "Cette ville est utilisé dans une commande",
                };
            } else {
                return {
                    code: 400,
    ...
    //New function - Talha
    static async GetTeamMemberList({ id_user, isHidden }) {
        try {
            var team = await Team_User.findAll({
                include: [
                    {
                        ...
                        attributes: [],
                        where: {
                            updownsell: 'UpSell', // Filter by the updownsell attribute
                            id_team: sequelize.col('Team_User.id')
                        }, // Add the association condition
                        required: false, // Use 'required: false' if the association is optional
                    },
                ],
                ...
                attributes: {
                    exclude: ['password'],
                    include: [
                        [sequelize.literal("(SELECT COUNT(*) FROM `Orders` WHERE `Orders`.`id_team` = `Team_User`.`id` AND (`Orders`.`status` = 'Paye' OR `Orders`.`status` = 'Livre'))"), 'totalOrderCount'],
                        //[sequelize.literal("(SELECT COUNT(*) FROM `Orders` WHERE `Orders`.`id_team` = `Team_User`.`id` AND `Orders`.`updownsell` = 'UpSell')"), 'upSellOrderCount'],
                        [sequelize.literal("(SELECT COUNT(*) FROM `Orders` WHERE `Orders`.`id_team` = `Team_User`.`id` AND `Orders`.`updownsell` = 'UpSell' AND (`Orders`.`status` = 'Paye' OR `Orders`.`status` = 'Livre') )"), 'upSellOrderCount']
                    ],
                },
            ...
            try {
                var team = await Team_User.findAll({
                    ...
                    include: [
                        ...
                        {
                            model: Order,
                            attributes: ['id_team', 'updownsell'],
                        },
                    ],
                    where: { id_user, active: Boolean(isHidden) },
                });

                team = team.map(x => x.get({ plain: true }))

                team.forEach(
                    (teamMember) => {

                        let upsellOrdersCount = teamMember.Orders.filter(order => order.updownsell == 'UpSell').length;//upsell order count
                        let totalOrdersCount = teamMember.Orders.length;

                        //find total number of upsell orders
                        //upsell = total number of upsell orders / total number of orders 

                        let upsellValue = (upsellOrdersCount / totalOrdersCount) * 100;
                        upsellValue = (upsellValue && upsellValue > 0) ? (isFloat(upsellValue) ? upsellValue.toFixed(2) : upsellValue) : 0;
                        teamMember.upsellValue = upsellValue;
                    }
                );
    ...
    static async AddTeamMember({
        ...
        //console.log(all_product_access);
        ...
        const subscription = await this.#findSubscriptionByIdUser(id_user);
        if (!subscription)
            return { code: 404, message: "You don't have any subscription" };
        else {
            if (
                !isExist.isTrial ||
                getDayBetween(isExist.createdAt, new Date()) >=
                Number(isExist.Setting_Admin.trial_period)
            ) {
                if (
                    getDayBetween(isExist.trialAt, new Date()) >=
                    Number(isExist.trialPeriod)
                ) {
                    const item_inclued = subscription.Pack.item_inclued;
                    const max_team_permit =
                        item_inclued[2] !== "" ? Number(item_inclued[2]) : Infinity;

                    const count_team = await this.#countTeamByIdUser(id_user);

                    console.log(max_team_permit);
                    console.log(count_team);

                    if (count_team >= max_team_permit)
                        return {
                            code: 400,
        ...
        // verify Pack
        if (this.#verifyPackValidy({ id_user, transaction }) === 404)
            return { code: 400, message: "Your pack as expired" };
        ...
        var teamMember = Team_User.build({
            id_user,
            name,
            email,
            livoToken,
        ...
        try {
            ...
            console.log("=================ID TEAM================", id_team);

            if (!all_column_access) {
                column_access.map(async (id_column_of_order) => {
                    try {
                        var columnAccess = await Team_Client_Column_Acces.build({
                            id_team,
                            id_column_of_order,
                        });
                        await columnAccess
            ...
            if (!all_cities_access) {
                cities_access.map(async (id_city) => {
                    try {
                        var citiesAccess = await Team_Client_City_Acces.build({
                            id_team,
                            id_city,
                        });
                        await citiesAccess
            ...
            if (!all_product_access) {
                product_access.map(async (id_product) => {
                    try {
                        var productAccess = await Team_Client_Product_Acces.build({
                            id_team,
                            id_product,
                        });
                        await productAccess
            ...
            if (!all_page_access) {
                page_access.map(async (id_client_page) => {
                    try {
                        var pageAccess = await Team_Client_Page_Acces.build({
                            id_team,
                            id_client_page,
                        });
                        await pageAccess
    ...
    static async PatchTeamMember({
        ...
        if (!subscription)
            ...
            if (
                !client.isTrial ||
                getDayBetween(client.createdAt, new Date()) >=
                Number(client.Setting_Admin.trial_period)
            ) {
                if (
                    getDayBetween(client.trialAt, new Date()) >=
                    Number(client.trialPeriod)
                ) {
                    if (active) {
                        const item_inclued = subscription.Pack.item_inclued;
                        const max_team_permit =
                        ...
                        if (count_team >= max_team_permit)
                            return {
                                code: 400,
                                message:
                                    "You have reached the maximum number of active team members",
                            };
                    }
                }
            }
        ...
        isExist.all_column_access =
            active == undefined
                ? all_column_access ?? isExist.all_column_access
                : isExist.all_column_access;
        isExist.all_cities_access =
            active == undefined
                ? all_cities_access ?? isExist.all_cities_access
                : isExist.all_cities_access;
        isExist.all_product_access =
            active == undefined
                ? all_product_access ?? isExist.all_product_access
                : isExist.all_product_access;
        isExist.all_page_access =
            active == undefined
                ? all_page_access ?? isExist.all_page_access
                : isExist.all_page_access;
        ...
        try {
            if (active == false) {
                ...
                var orders = await Order.findAll({
                    where: { id_team: id, status: { [Op.notIn]: DESAC } },
                });
                ...
                if (orders.length === 0) {
                    ...
                    const countAvailableTeam = await this.#CountAvailableTeam({
                        current_id_team: isExist.id,
                        id_user,
                    });
                    if (countAvailableTeam < orders.length)
                        return {
                            code: 400,
                            message:
                                "Error, The number of orders for this member is greater than the number of available members",
                        };

                    // loop on orders and assign team to order
                    orders.map(async (order) => {
                        var team_user = await this.#SearchAvailableTeam({
                            current_id_team: isExist.id,
                            id_user,
                        });
                        if (!team_user)
                            return {
                                code: 404,
                                message:
                                    "Internal error contact the support, team_member(error)",
                            };
            ...
            return {
                code: error.code || 400,
                message: error.message || "Internal error, please contact support",
            };
        }
    }
    ...
    static async GiveFreeTrial({ id_user, period }) {
        ...
        try {
            if (
                ...
                return {
                    code: 404,
                    message: `A free period is currently running for this customer please wait ${Number(client.Setting_Admin.trial_period) -
                        getDayBetween(client.createdAt, new Date())
                        } days`,
                };
            }
    ...
    static async AddProduct({
        ...
        if (isExistProduct)
            return {
                code: 404,
                message: "The name is already take by other product",
            };

        var product = await Product.build({
            id_user,
            name,
            variant,
            price_selling,
        ...
        try {
            ...
            if (other_charges_array.length > 0) {
                other_charges_array.map(async (otc) => {
                    const othercharge = await Other_Charge.build({
                        name: otc.name,
                        price: otc.price,
                        id_product: productSaved.id,
                    });
    ...
    static async PatchProduct({
        ...
        isExist.price_best_selling =
            price_best_selling ?? isExist.price_best_selling;
    ...
    static async RemoveProduct({ id, id_user }) {
        ...
        if (pr)
            return {
                code: 400,
                message: "You can't this product already exist in transaction",
            };
    ...
    static async GetSetting({ id_user }) {
        try {
            ...
            return {
                ...
                data: {
                    ...isExist.dataValues,
                    trial_period: settAdm.Setting_Admin.trial_period,
                },
            };
        } catch (error) {
            console.log(error);
    ...
    static async GetShippingCompanie({ id_user }) {
        ...
        try {
            ...
            var companies = await Shipping_Companie.findAll({
                order: [["range", "ASC"]],
            });
    ...
    static async PatchSetting({
        ...
        var setting = Setting_User.build({
            ...
            delfaulnpt_del_pricing:
                delfaulnpt_del_pricing ?? isExist.delfaulnpt_del_pricing,
    ...
    static async PatchStatus({ id, id_user, name, checked, color }) {
        ...
        try {
            ...
            if (checked == false) {
                ...
                if (PROHIBITED_STATUS.includes(isExist.name))
                    return { code: 404, message: "You can`t disable this status" };
                ...
                if (statusExist)
                    return { code: 404, message: "This status is already in use" };
    ...
    static async GetStatus({
        ...
        try {
            ...
            if (search) {
                options.where.push(
                    sequelize.where(
                        sequelize.fn(
                            "concat",
                            sequelize.col("Order.nom"),
                            sequelize.col("Order.telephone"),
                            sequelize.col("Order.prix"),
                            sequelize.col("Order.status"),
                            sequelize.col("Order.adresse"),
                            sequelize.col("Order.source"),
                            sequelize.col("Order.updownsell"),
                            sequelize.col("City_User.name"),
                            sequelize.col("Team_User.name"),
            ...
            //     const count = await Order.count({ ...options, distinct: true });
            //     countOrderByStatus[i].count = count;
            // }


            const countPromises = countOrderByStatus.map(async (status) => {
                options.where[0].status = status.name;
                const count = await Order.count({ ...options, distinct: true });
                return { ...status, count };
            });

            const countOrderByStatusWithCounts = await Promise.all(countPromises);


            return { code: 200, data: isExist, countOrderByStatus: countOrderByStatusWithCounts };
        } catch (error) {
            return {
                code: 400,
                message: "Internal error, please contact the support",
    ...
    static async MakeSubscription({ id_pack, id_user }) {
        ...
        var subscription = await Subscription.build({
            date_subscription: date[0],
            date_expiration: date[1],
            id_pack,
            id_user,
        });
    ...
    static async ChangeSubscription({ id, id_pack, id_user }) {
        ...
        try {
            ...
            if (!isExist)
                throw { code: 404, message: "This ressource doesn't exist" };
            ...
            if (isExist.Pack.price_per_month > account.solde)
                throw {
                    code: 400,
                    message: "You don't have enough money to change your subscription",
                };
            ...
            if (nb_team > max_team_permit)
                throw { code: 400, message: "Your must to disable some team member" };
    ...
    static async AddOrder({
        ...
        try {
            ...
            if (!subscription)
                ...
                if (
                    ...
                    if (
                        ...
                        const max_order_permit =
                            item_inclued[3] !== "" ? Number(item_inclued[3]) : Infinity;

                        const count_order = await this.#countOrderByIdUser(id_user);

                        if (count_order >= max_order_permit)
                            return {
                                code: 404,
                                message: "You have reached the maximum number of orders",
                            };
            ...
            // Order process
            var team_user = await this.#SearchAvailableTeamByProduct({
                id_user,
                id_products: getIdsProduct({ id_product_array }),
            });

            if (SheetId) {
                var sheetExist = await this.#findOrderByIdSheet(SheetId);
                if (sheetExist)
                    return { code: 404, message: "This sheet already exist" };
            }
            ...
            //if order is not posted by admin
            if (typeof userWhoPostedOrder?.id_team != "undefined") {
                id_team = userWhoPostedOrder.id_team;
            } else if (team_user) {
                var random = Math.floor(Math.random() * team_user.length);
            ...
            let isOrderUpsett = id_product_array.some(item => Number(item.quantity) >= 2);
            ...
            const order = await Order.build({
                ...
                source,
                id_team,
                updownsell: isOrderUpsett ? "UpSell" : updownsell,
                date,
                message,
                changer,
                ouvrir,
            ...
            const commentBuild = await Order_Comment.build({
                message,
                id_order: orderSaved.id,
            });
            ...
            for (let i = 0; i < id_product_array.length; i++) {
                ...
                try {
                    var product_order = await Product_Order.build({
                        id_order: orderSaved.id,
    ...
    static async GetOrder({
        ...
        async function getOrderByDefault(
            id_user,
            id_team,
            id_product_array,
            _skip,
            ...
            try {
                var order = await Order.findAll(
                    id_product_array.length > 0 ? search_params : search_params_basic
                );
                return order;
            } catch (error) {
        ...
        async function getOrderBySearch(
            id_user,
            id_team,
            search,
            id_product_array,
        ...
        try {
            if (!usedate) {
                    ...
                    for (let j = 0; j < columnFound.length; j++) {
                        ...
                        switch (columnFound[j].name) {
                            ...
                            case "Telephone":
                                ...
                                break;
                            case "Ville":
                                value = order[i].City_User
                                    ? order[i].City_User.name
                                    : "undifined";
                                break;
                            case "Changer":
                ...
                var order = await getOrderByDate(
                    id_user,
                    id_team,
                    datefrom,
                    dateto,
    ...
    static async DeleteBulkOrder({ id_orders, id_user }) {
        ...
        try {
            ...
            const result = await Order.update(
                { status: "deleted", id_city: null, id_team: null, isDeleted: true },
                {
                    where: { id: id_orders },
                    transaction,
                }
    ...
    static async EditBulkOrderTeam({ id_orders, id_user, new_id_team }) {
        ...
        try {
            ...
            if (new_team.nb_order >= new_team.max_order)
                throw {
                    code: 400,
                    message: "This team member has reached its maximum number of orders",
                };
            if (!new_team.active)
                throw { code: 400, message: "This team member is not active" };
            if (new_team.nb_order + id_orders.length >= new_team.max_order)
                throw {
                    code: 400,
                    message: `you cant give ${id_orders.length} order(s) to this team the remaining number is insufficient`,
                };
            ...
            orders.map(async (odr) => {
                if (DELIVRED.includes(odr.status)) {
                    ...
                    const teamAlreadyPatched = await this.#findAuditByMessage({
                        message: await this.#getTeamNameById(odr.id_team),
                        id_order: odr.id,
                    });
            ...
            const teamAlreadyPatched = await this.#findAuditByMessage({
                message: await this.#getTeamNameById(new_team.id),
                id_order: id_orders,
            });
    ...
    static async PatchOrder(orderData) {
        const {
            id,
            id_user,
            date,
        ...
        // console.log("updownsell:", updownsell);
        // console.log("prev_id_team:", prev_id_team);
        // console.log("id_team:", id_team);
        // console.log("id_city:", id_city);
        // console.log("id_product_array:", id_product_array);
        // console.log("message:", message);
        // console.log("changer:", changer);
        // console.log("ouvrir:", ouvrir);
        ...
        //id_team = id_team == 0 ? null : id_team;
        isExist.id_team = id_team == 0 ? null : (id_team ?? isExist.id_team);
        isExist.id_city = id_city ?? isExist.id_city;
        isExist.date = date ?? isExist.date;
        isExist.reportedDate =
            isExist.status === "Reporte"
                ? reportedDate ?? isExist.reportedDate
                : null;
        ...
        try {
            ...
            if (
                ...
                if (isExist.LivoId) {
                    var response = await this.#PatchLivoOrder({
                        data: { status: final_status },
                        token: client.livoToken,
                        LivoId: isExist.LivoId,
                    });
            ...
            const orderClosed = await this.#findAuditByMessage({
                message: "DELIVRED",
                id_order: id,
            });
            ...
            if (!orderClosed) {
                ...
                if (status) {
                    const subscription = await this.#findSubscriptionByIdUser(id_user);
                    if (!subscription)
                        throw { code: 404, message: "You don't have any subscription" };
                    else {
                        if (
                            !client.isTrial ||
                            getDayBetween(client.createdAt, new Date()) >=
                            ...
                            try {
                                ...
                                if (["Livre", "Paye"].includes(status)) {
                                    ...
                                    const teamAlreadyPatched1 = await this.#findAuditByMessage({
                                        message: "Livre",
                                        id_order: id,
                                    });
                                    const teamAlreadyPatched2 = await this.#findAuditByMessage({
                                        message: "Paye",
                                        id_order: id,
                                    });
                ...
                if (PENDING.includes(status ?? isExist.status)) {
                    const teamAlreadyPatched = await this.#findAuditByMessage({
                        message: await this.#getTeamNameById(isExist.id_team),
                        id_order: id,
                    });
                    if (!teamAlreadyPatched) {
                        if (prev_id_team) {
                                ...
                                if (new_team) {
                                    if (new_team.nb_order >= new_team.max_order)
                                        throw {
                                            code: 400,
                                            message:
                                                "This team member has reached its maximum number of orders",
                                        };
                                    if (!new_team.active)
                                        throw {
                                            code: 400,
                                            message: "This team member is not active",
                                        };
                ...
                if (id_product_array) {
                    const item_del = await Product_Order.findOne({
                        where: { id_order: id },
                    });
                ...
                var audited = await this.AddAuditOrder({
                    id_order: id,
                    status,
                    id_city,
                    source,
                ...
                if (!audited)
                    throw {
                        code: 400,
                        message: "Internal errors, please contact the support",
                    };
    ...
    static async UseCoupon({ id_user, code }) {
        ...
        if (coupon.client_used.includes(id_user))
            return { code: 400, message: "You have already use this coupon" };

        if (coupon.used >= coupon.limit)
            return { code: 400, message: "This coupon is no longer available" };
    ...
    static async GetDashboard({
        ...
        try {
            var costPerLead = await this.getCostPerLead({
                id_user,
                id_team,
                dateFrom,
                dateTo,
            ...
            var totalOrder = await this.getTotalOrder({
                id_user,
                id_team,
                dateFrom,
                dateTo,
            ...
            var upsellRate = await this.getUpsellRate({
                id_user,
                id_team,
                dateFrom,
                dateTo,
            ...
            var crosssellRate = await this.getCrossSellRate({
                id_user,
                id_team,
                dateFrom,
                dateTo,
            ...
            var orderInProgress = await this.getOrderInProgress({
                id_user,
                id_team,
                dateFrom,
                dateTo,
            ...
            var costPerDelivred = await this.getCostPerDelivred({
                id_user,
                id_team,
                dateFrom,
                dateTo,
            ...
            var rateOfConfirmed = await this.getRateOfConfirmed({
                id_user,
                id_team,
                dateFrom,
                dateTo,
            ...
            var rateOfDelivred = await this.getRateOfDelivred({
                id_user,
                id_team,
                dateFrom,
                dateTo,
            ...
            var bestSellingProduct = await this.getBestSellingProduct({
                id_user,
                id_team,
                dateFrom,
                dateTo,
            ...
            var bestCity = await this.getBestCity({
                id_user,
                id_team,
                dateFrom,
                dateTo,
            ...
            var orderReport = await this.getOrderReport({
                id_user,
                id_team,
                dateFrom,
                dateTo,
            ...
            var orderStatistic = await this.getOrderStatistics({
                id_user,
                id_team,
                dateFrom,
                dateTo,
            ...
            var rateReport = await this.getRateReport({
                id_user,
                id_team,
                dateFrom,
                dateTo,
            ...
            var reportEarningNet = await this.getReportEarningNet({
                id_user,
                id_team,
                dateFrom,
                dateTo,
            ...
            if (
                ...
                costPerDelivred === null ||
                rateOfConfirmed === null ||
                rateOfDelivred === null ||
                ...
                rateReport === null ||
                earningNet === null ||
                reportEarningNet === null ||
                stock === null ||
                upsellRate === null ||
                crosssellRate === null
            )
                console.log("=========================");
            console.log(costPerDelivred === null);
            console.log(rateOfConfirmed === null);
            console.log(rateOfDelivred === null);
            console.log(bestSellingProduct === null);
            console.log(bestCity === null);
            console.log(orderReport === null);
            console.log(orderStatistic === null);
            console.log(totalOrder === null);
            console.log(costReport === null);
            console.log(rateReport === null);
            console.log(earningNet === null);
            console.log(reportEarningNet === null);
            console.log(stock === null);
            console.log(upsellRate === null);
            console.log(crosssellRate === null);
    ...
    static async getUpsellRate({
        ...
        try {
            if (!useDate) {
                ...
                var total_order = await Order.count(
                    id_product_array.length > 0
                ...
                var upsell_order = await Order.count(
                    id_product_array.length > 0
                        ? search_params_with_product
                        : search_params_basic
                );
    ...
    static async getCrossSellRate({
        ...
        try {
            if (!useDate) {
                ...
                var crosssell_order = await Order.count(
                    id_product_array.length > 0
                        ? search_params_with_product
                        : search_params_basic
                );
    ...
    static async getCostPerLead({
        ...
        try {
            if (!useDate) {
                ...
                var nb_order = await Order.count(
                    id_product_array.length > 0
                        ? search_params_with_product
                        : search_params_basic
                );
    ...
    static async getOrderInProgress({
        id_user,
        id_team,
        dateFrom,
        dateTo,
    ...
    static async getCostPerDelivred({
        id_user,
        id_team,
        dateFrom,
        dateTo,
    ...
    static async getRateOfConfirmed({
        id_user,
        id_team,
        dateFrom,
        dateTo,
    ...
    static async getRateOfDelivred({
        id_user,
        id_team,
        dateFrom,
        dateTo,
    ...
    static async getBestSellingProduct({
        id_user,
        id_team,
        dateFrom,
        dateTo,
            ...
            if (!useDate) {
                ...
                for (let i = 0; i < product.length; i++) {
                    ...
                    products.push({
                        ...
                        price: 0,
                        count: 0,
                        price_product: element.price_selling,
                    });
                }
        ...
        try {
            if (!useDate) {
                var search_params_with_product = {
                    ...
                    attributes: [
                        "id_city",
                        [sequelize.fn("COUNT", sequelize.col("id_city")), "count"],
                    ],
                };
    ...
    static async getOrderReport({
        ...
        async function getReportByDays(
            days,
            endDate,
            status,
            id_product_array,
        ...
        async function getReportByWeek(
            startDate,
            endDate,
            status,
            id_product_array,
        ...
        try {
            if (!useDate) {
                ...
                const delivred = await getReportByDays(
                    7,
                    new Date(),
                    DELIVRED,
                    id_product_array,
                ...
                const canceled = await getReportByDays(
                    7,
                    new Date(),
                    CANCELED,
                    id_product_array,
                ...
                const pending = await getReportByDays(
                    7,
                    new Date(),
                    PENDING,
                    id_product_array,
                    ...
                    datasets: [
                        {
                            label: "Delivred",
                            data: delivred
                                ? delivred.map((item) => item.count).reverse()
                        ...
                        },
                        {
                            label: "Canceled",
                            data: canceled
                                ? canceled.map((item) => item.count).reverse()
                                : Array(formatedDate.length).fill(0),
                            fill: false,
                        ...
                        },
                        {
                            label: "Pending",
                            data: pending
                                ? pending.map((item) => item.count).reverse()
                                : Array(formatedDate.length).fill(0),
                            fill: false,
                ...
                if (differenceInDays < 14) {
                    ...
                    const delivred = await getReportByWeek(
                        dateFrom,
                        dateTo,
                        DELIVRED,
                        id_product_array,
                    ...
                    const canceled = await getReportByWeek(
                        dateFrom,
                        dateTo,
                        CANCELED,
                        id_product_array,
                    ...
                    const pending = await getReportByWeek(
                        dateFrom,
                        dateTo,
                        PENDING,
                        id_product_array,
                    ...
                    const delivred = await getReportByMonth(
                        dateFrom,
                        dateTo,
                        DELIVRED,
                        id_product_array,
                    ...
                    const canceled = await getReportByMonth(
                        dateFrom,
                        dateTo,
                        CANCELED,
                        id_product_array,
                    ...
                    const pending = await getReportByMonth(
                        dateFrom,
                        dateTo,
                        PENDING,
                        id_product_array,
    ...
    static async getOrderStatistics({
        ...
        try {
            if (!useDate) {
                ...
                const delivred = await Order.count(
                    id_product_array.length != 0
                        ? search_params_with_product
                ...
                const canceled = await Order.count(
                    id_product_array.length != 0
                        ? search_params_with_product
                        : search_params_without_product
                );
                ...
                const pending_injoignable = await Order.count(
                    id_product_array.length != 0
                        ? search_params_with_product
                        : search_params_without_product
                );
                ...
                const deleted = await Order.count(
                    id_product_array.length != 0
                        ? search_params_with_product
                        : search_params_without_product
                ); // 'deleted
                ...
                // sum of all orders
                // const total =
                //     delivred + canceled + pending + pending_injoignable + deleted;
                const total =
                    delivred + canceled + pending + pending_injoignable;
    ...
    static async getCostReport({
        ...
        async function getCostPerLeadByDays(
            days,
            endDate,
            id_product_array,
            id_team
            ...
            for (let i = 0; i < pertes.length; i++) {
                ...
                orders_date.push({
                    date: date,
                    dateTo: dateTo,
                    sum: element.amount / (element.dataValues.dateDifference + 1),
                });
            }
            ...
            var value = await getReportByDays(
                days,
                endDate,
                [...DELIVRED, ...CANCELED, ...PENDING],
                id_product_array,
                id_team
            ).then((arr2) => {
        ...
        async function getCostPerLeadByWeek(
            startDate,
            endDate,
            id_product_array,
            id_team
            ...
            try {
                ...
                const report = await getReportByWeek(
                    startDate,
                    endDate,
                    [...DELIVRED, ...CANCELED, ...PENDING],
                    id_product_array,
        ...
        async function getCostPerLeadByMonth(
            startDate,
            endDate,
            id_product_array,
            id_team
            ...
            try {
                ...
                const report = await getReportByMonth(
                    startDate,
                    endDate,
                    [...DELIVRED, ...CANCELED, ...PENDING],
                    id_product_array,
        ...
        async function getCostPerDelivredByDays(
            days,
            endDate,
            id_product_array,
            id_team
        ...
        async function getCostPerDelivredByWeek(
            startDate,
            endDate,
            id_product_array,
            id_team
        ...
        async function getCostPerDelivredByMonth(
            startDate,
            endDate,
            id_product_array,
            id_team
        ...
        try {
            if (!useDate) {
                ...
                var costPerLead = await getCostPerLeadByDays(
                    7,
                    new Date(),
                    id_product_array,
                    id_team
                );
                var costPerDelivred = await getCostPerDelivredByDays(
                    7,
                    new Date(),
                    id_product_array,
                    id_team
                    ...
                    datasets: [
                        {
                            label: "Cost per lead",
                            data: costPerLead
                                ? costPerLead.map((item) => item.sum).reverse()
                                : Array(formatedDate.length).fill(0),
                            fill: false,
                            borderColor: "rgb(75, 192, 192)",
                            tension: 0.1,
                        },
                        {
                            ...
                            data: costPerDelivred
                                ? costPerDelivred.map((item) => item.sum).reverse()
                                : Array(formatedDate.length).fill(0),
                            fill: false,
                            borderColor: "rgb(255, 99, 132)",
                            tension: 0.1,
                ...
                if (differenceInDays < 14) {
                    ...
                    var costPerLead = await getCostPerLeadByWeek(
                        dateFrom,
                        dateTo,
                        id_product_array,
                        id_team
                    );
                    var costPerDelivred = await getCostPerDelivredByWeek(
                        dateFrom,
                        dateTo,
                        id_product_array,
                        id_team
                    ...
                    var data = {
                        ...
                        datasets: [
                            ...
                            },
                            {
                                label: "Cost per delivred",
                                data:
                                    costPerDelivred.length != 0
                                        ? weeks.map((date) => {
                                            var amount = costPerDelivred.find(
                                                (element) => element.date == date
                                            );
                                            return amount ? amount.amount : 0;
                                        })
                    ...
                    var costPerLead = await getCostPerLeadByMonth(
                        dateFrom,
                        dateTo,
                        id_product_array,
                        id_team
                    );
                    var costPerDelivred = await getCostPerDelivredByMonth(
                        dateFrom,
                        dateTo,
                        id_product_array,
                        id_team
    ...
    static async GetTeamDashboard({
        ...
        if (!Number(id_team)) {
            try {
                var performance = await this.GetAllTeamPerformance({
                    id_user,
                    useDate,
                    dateFrom,
                    dateTo,
                });
                var performance_rate = await this.GetAllTeamPerformanceRate({
                    id_user,
                    useDate,
                    dateFrom,
                    dateTo,
                });
                var earning = await this.GetAllTeamEarning({
                    id_user,
                    useDate,
                    dateFrom,
                    dateTo,
                });
                var earning_table = await this.GetAllTeamEarningTable({
                    id_user,
                    id_team,
                    useDate,
                    dateFrom,
                ...
                // if (!performance || !earning || !earning_table || !performance_rate)
                //     return { code: 500, message: "Internal server error" };
        ...
        try {
            var performance = await this.GetTeamPerformance({
                id_user,
                id_team,
                useDate,
                dateFrom,
            ...
            var performance_rate = await this.GetTeamPerformanceRate({
                id_user,
                id_team,
                useDate,
                dateFrom,
            ...
            var earning = await this.GetTeamEarning({
                id_user,
                id_team,
                useDate,
                dateFrom,
            ...
            var earning_table = await this.GetTeamEarningTable({
                id_user,
                id_team,
                useDate,
                dateFrom,
    ...
    static async GetTeamPerformance({
        ...
        try {
            ...
            if (!useDate) {
                ...
                var count_status_delivred = await Order.count({
                    where: { id_user, status: DELIVRED, id_team },
                });

                var count_status_canceled = await Order.count({
                    where: { id_user, status: CANCELED, id_team },
                });

                var count_status_pending = await Order.count({
                    where: { id_user, status: PENDING, id_team },
                });

                var count_status_injoignable = await Order.count({
                    where: { id_user, status: INJOIGNABLE, id_team },
                });

                var count_status_deleted = await Order.count({
                    where: { id_user, status: "deleted", id_team },
                });

                const data = {
                    //labels: ["delivred", "canceled", "pending", "injoignable", "deleted"],
                    labels: ["delivred", "canceled", "pending", "injoignable"],
                    datasets: [
                        {
                            label: "Performance",
                            data: [
                                count_status_delivred,
                                count_status_canceled,
                                count_status_pending,
                                count_status_injoignable,
                                //count_status_deleted,
                            ],
                            fill: true,
                            backgroundColor: [
                                "rgb(67,160,71)",
    ...
    static async GetTeamPerformanceRate({
        id_user,
        id_team,
        useDate,
        dateFrom,
        ...
        try {
            ...
            if (!useDate) {
                    ...
                    datasets: [
                        {
                            label: "Performance",
                            data: [
                                computeRate(count_order, count_status_delivred),
                                computeRate(count_order, count_status_canceled),
                                computeRate(count_order, count_status_pending),
                                computeRate(count_order, count_status_injoignable),
                                //computeRate(count_order, count_status_deleted),
                            ],
                            fill: true,
                            backgroundColor: [
                                "rgb(67,160,71)",
                ...
                var count_order = await Order.count({
                    where: {
                        id_user,
                        createdAt: { [Op.between]: [dateFrom, dateTo] },
                        id_team,
    ...
    static async GetTeamEarning({ id_user, id_team, useDate, dateFrom, dateTo }) {
        ...
        /** Running function */
        async function getTeamEarningByMonth(
            id_user,
            startDate,
            endDate,
            id_team,
            ...
            try {
                ...
                for (let i = 0; i < result.length; i++) {
                    ...
                    result_formated.push({
                        date: date.toISOString().slice(0, 10),
                        amount: element.count * team.commission + team.salaire,
                    });
                }
        ...
        async function getTeamEarningByDays(id_user, days, endDate, id_team, team) {
            ...
            for (let i = 0; i < orders.length; i++) {
                ...
                orders_date.push({
                    date: date.toISOString().slice(0, 10),
                    amount:
                        element.dataValues.count * team.commission +
                        (days * team.salaire) / 30,
                });
            }
        ...
        async function getTeamEarningByWeek(
            id_user,
            days,
            startDate,
            endDate,
            ...
            try {
                ...
                for (let i = 0; i < result.length; i++) {
                    ...
                    result_formated.push({
                        date: date.toISOString().slice(0, 10),
                        amount:
                            element.count * team.commission + (days * team.salaire) / 30,
                    });
                }
        ...
        if (!useDate) {
            ...
            var months = getStartDateBetweenMonth(
                start_end_date[0],
                start_end_date[1]
            );
            var earning = await getTeamEarningByMonth(
                id_user,
                start_end_date[0],
                start_end_date[1],
                id_team,
            ...
            if (differenceInDays < 14) {
                const earning = await getTeamEarningByDays(
                    id_user,
                    differenceInDays,
                    new Date(dateTo),
                    id_team,
                    ...
                    datasets: [
                        {
                            label: "Earning",
                            data: earning
                                ? earning.map((item) => item.amount).reverse()
                                : Array(formatedDate.length).fill(0),
                            fill: false,
                            borderColor: "rgb(255,255,0)",
                            tension: 0.1,
                ...
                const earning = await getTeamEarningByWeek(
                    id_user,
                    differenceInDays,
                    dateFrom,
                    dateTo,
                ...
                const earning = await getTeamEarningByMonth(
                    id_user,
                    dateFrom,
                    dateTo,
                    id_team,
    ...
    static async GetTeamEarningTable({
        id_user,
        id_team,
        useDate,
        dateFrom,
        ...
        try {
            ...
            if (!useDate) {
                ...
                var her_earning_downsell = await Order.count(
                    her_earning_downsell_params
                );
                var her_earning_crosssell = await Order.count(
                    her_earning_crosssell_params
                );

                const data = {
                    livre: {
                        nb_commande: livre,
                        her_earning: livre * team.commission,
                        salaire: team.salaire,
                    },
                    upsell: {
                        nb_commande: upsell,
                        her_earning: her_earning_upsell * team.commission,
                        salaire: team.salaire,
                    },
                    downsell: {
                        nb_commande: downsell,
                        her_earning: her_earning_downsell * team.commission,
                        salaire: team.salaire,
                    },
                    crosssell: {
                        nb_commande: crosssell,
                        her_earning: her_earning_crosssell * team.commission,
                        salaire: team.salaire,
                    },
                };
                ...
                const data = {
                    livre: {
                        nb_commande: livre,
                        her_earning: livre * team.commission,
                        salaire: (differenceInDays * team.salaire) / 30,
                    },
                    upsell: {
                        nb_commande: upsell,
                        her_earning: her_earning_upsell * team.commission,
    ...
    static async GetAllTeamPerformance({ id_user, useDate, dateFrom, dateTo }) {
        try {
            ...
            if (!useDate) {
                // count order group by status left join with status
                var count_status_delivred = await Order.count({
                    where: { id_user, status: DELIVRED },
                });

                var count_status_canceled = await Order.count({
                    where: { id_user, status: CANCELED },
                });
                ...
                var count_status_injoignable = await Order.count({
                    where: { id_user, status: INJOIGNABLE },
                });

                var count_status_deleted = await Order.count({
                    where: { id_user, status: "deleted" },
                });
    ...
    static async GetAllTeamPerformanceRate({
        id_user,
        useDate,
        dateFrom,
        dateTo,
        ...
        try {
            ...
            if (!useDate) {
                ...
                var count_order = await Order.count({
                    where: { id_user, createdAt: { [Op.between]: [dateFrom, dateTo] } },
                });
        ...
        async function getTeamEarningByWeek(id_user, startDate, endDate) {
            ...
            try {
                const commission = await Team_User.findAll({
                    attributes: [
                        [
                            sequelize.fn("YEARWEEK", sequelize.col("Orders.createdAt"), 1),
                            "week",
                        ],
                        [sequelize.fn("sum", sequelize.col("commission")), "total"],
                    ],
    ...
    static async GetAllTeamEarningTable({ id_user, useDate, dateFrom, dateTo }) {
        ...
        try {
            ...
            if (!useDate) {
                ...
                var sum_salary = await Team_User.sum("salaire", {
                    where: { id_user: id_user, active: true },
                });

                const data = {
                    livre: {
                        nb_commande: livre,
                        her_earning: sum_commission,
                        salaire: sum_salary,
                    },
                    upsell: {
                        nb_commande: upsell,
                        her_earning: sum_upsell,
                        salaire: sum_salary,
                    },
                    downsell: {
                        nb_commande: downsell,
                        her_earning: sum_downsell,
                        salaire: sum_salary,
                    },
                    crosssell: {
                        nb_commande: crosssell,
                        her_earning: sum_crosssell,
                        salaire: sum_salary,
                    },
                };
                ...
                const data = {
                    livre: {
                        nb_commande: livre,
                        her_earning: sum_commission,
                        salaire: (differenceInDays * sum_salary) / 30,
                    },
                    upsell: {
                        nb_commande: upsell,
                        her_earning: sum_upsell,
    ...
    static async getRateReport({
        ...
        async function getRateOfDelivredByDays(
            days,
            endDate,
            id_product_array,
            id_team
            ...
            var nb_order = await Order.findAll(
                id_product_array.length != 0
                    ? search_params_with_product
                    : search_params_without_product
            );
            ...
            for (let i = 0; i < orders.length; i++) {
                ...
                for (let j = 0; j < nb_order.length; j++) {
                    ...
                    if (date == date_order) {
                        orders_date.push({
                            date,
                            sum: (element.dataValues.count * 100) / order.dataValues.count,
                        });
                    }
                }
            }
        ...
        async function getRateOfConfirmedByDays(
            days,
            endDate,
            id_product_array,
            id_team
        ...
        async function getRateByWeek(
            ...
            try {
                ...
                const result_nb_order = await seqlz.query(
                    id_product_array.length != 0
                        ? query_with_product_wStatus
                        : query_without_product_wStatus,
                    {
                        replacements: {
                            startDate,
                            endDate,
                ...
                for (let i = 0; i < result.length; i++) {
                    ...
                    result_formated.push({
                        date: date.toISOString().slice(0, 10),
                        count: (element.count * 100) / element_nb_order.count,
                    });
                }
        ...
        try {
            if (!useDate) {
                ...
                var rateOfDelivred = await getRateOfDelivredByDays(
                    7,
                    new Date(),
                    id_product_array,
                    id_team
                );
                var rateOfConfirmed = await getRateOfConfirmedByDays(
                    7,
                    new Date(),
                    id_product_array,
                    id_team
                    ...
                    datasets: [
                        {
                            ...
                            data: rateOfDelivred
                                ? rateOfDelivred.map((item) => item.sum).reverse()
                                : Array(formatedDate.length).fill(0),
                            fill: false,
                            borderColor: "rgb(75, 192, 192)",
                            tension: 0.1,
                        },
                        {
                            ...
                            data: rateOfConfirmed
                                ? rateOfConfirmed.map((item) => item.sum).reverse()
                                : Array(formatedDate.length).fill(0),
                            fill: false,
                            borderColor: "rgb(255, 99, 132)",
                            tension: 0.1,
                ...
                if (differenceInDays < 14) {
                    ...
                    var rateOfDelivred = await getRateByWeek(
                        dateFrom,
                        dateTo,
                        DELIVRED,
                        id_product_array,
                    ...
                    var rateOfConfirmed = await getRateByWeek(
                        dateFrom,
                        dateTo,
                        CONFIRMED,
                        id_product_array,
                        ...
                        datasets: [
                            {
                                label: "Rate of delivred",
                                data:
                                    rateOfDelivred.length != 0
                                        ? weeks.map((date) => {
                                            var amount = rateOfDelivred.find(
                                                (element) => element.date == date
                                            );
                            ...
                            },
                            {
                                label: "Rate of confirmed",
                                data:
                                    rateOfConfirmed.length != 0
                                        ? weeks.map((date) => {
                                            var amount = rateOfConfirmed.find(
                                                (element) => element.date == date
                                            );
                                            return amount ? amount.count : 0;
                                        })
                    ...
                    var rateOfDelivred = await getRateByMonth(
                        dateFrom,
                        dateTo,
                        DELIVRED,
                        id_product_array,
                    ...
                    var rateOfConfirmed = await getRateByMonth(
                        dateFrom,
                        dateTo,
                        CONFIRMED,
                        id_product_array,
    ...
    static async AddVariant({ id_user, name }) {
        ...
        try {
            ...
            if (varExisting)
                return { code: 400, message: "This variant already exist" };
    ...
    static async PatchVariant({ id_user, id, name }) {
        ...
        try {
            ...
            if (!varExisting)
                return { code: 400, message: "This variant doesn't exist" };
    ...
    static async GetPaiementDashbord({
        id_user,
        useDate,
        dateFrom,
        dateTo,
        ...
        try {
            var ChffAffaire = await this.GetChiffreAffaire({
                id_user,
                useDate,
                dateFrom,
                dateTo,
            ...
            var spending = await this.getSpending({
                id_user,
                useDate,
                dateFrom,
                dateTo,
            ...
            var spending_ads = await this.getSpendingAds({
                id_user,
                useDate,
                dateFrom,
                dateTo,
            ...
            var spending_product = await this.getSpendingProduct({
                id_user,
                useDate,
                dateFrom,
                dateTo,
            ...
            var spending_city = await this.getSpendingCity({
                id_user,
                useDate,
                dateFrom,
                dateTo,
            ...
            var spending_commission = await this.getSpendingCommission({
                id_user,
                useDate,
                dateFrom,
                dateTo,
            ...
            var spending_landing_design = await this.getSpendingLandingDesign({
                id_user,
                useDate,
                dateFrom,
                dateTo,
            ...
            var spending_autre = await this.getSpendingAutre({
                id_user,
                useDate,
                dateFrom,
                dateTo,
            ...
            var earningNet = await this.getEarningNet({
                id_user,
                useDate,
                dateFrom,
                dateTo,
            ...
            var detailOfSpending = await this.GetDetailsOfSpending({
                id_user,
                useDate,
                dateFrom,
                dateTo,
            ...
            if (
                ...
                earningNet === null ||
                transaction === null ||
                detailOfSpending === null ||
                spending_ads === null ||
                spending_product === null ||
                spending_city === null ||
                spending_commission === null ||
                spending_landing_design === null ||
                spending_autre === null
            )
                return { code: 500, message: "Internal server error" };
    ...
    static async GetChiffreAffaire({
        id_user,
        useDate,
        dateFrom,
        dateTo,
        ...
        if (!useDate) {
            ...
            var sumOrder = await Order.sum(
                "prix",
                id_product_array.length > 0
                    ? sum_order_with_product
                    : sum_order_without_product
            );
            sumOrder = sumOrder ?? 0;
    ...
    static async getSpending({
        ...
        if (!useDate) {
            try {
                ...
                var orders = await Order.findAll(
                    id_product_array.length > 0
                        ? order_params_with_product
                        : order_params_without_product
                );
                ...
                var pertes = await Perte.findAll(
                    id_product_array.length > 0
                        ? pertes_params_with_product
                        : pertes_params_without_product
                );

                var sum_city_user = 0;
                orders.forEach((ord) => {
                    var amount =
                        ord.City_User.price ?? ord.Setting_User.delfaulnpt_del_pricing;
                ...
                orders.forEach((ord) => {
                    var cms =
                        ord.Team_User.commission ?? ord.Setting_User.default_cof_ricing;
            ...
            try {
                ...
                var pertes = await Perte.findAll(
                    id_product_array.length > 0
                        ? perte_params_with_product
                        : perte_params_without_product
                );
                ...
                pertes.forEach((ptr) => {
                    ...
                    if (
                        ptr.dateFrom < new Date(dateFrom) &&
                        ptr.dateTo < new Date(dateTo)
                    ) {
    ...
    static async getSpendingProduct({
        id_user,
        useDate,
        dateFrom,
        dateTo,
    ...
    static async getSpendingCommission({
        id_user,
        useDate,
        dateFrom,
        dateTo,
    ...
    static async getSpendingLandingDesign({
        id_user,
        useDate,
        dateFrom,
        dateTo,
    ...
    static async getEarningNet({
        ...
        if (!useDate) {
            try {
                ...
                var commission = await Order.findAll(
                    id_product_array.length > 0
                        ? order_params_with_product
                        : order_params_without_product
                );
                ...
                var sum_prix_order = await Order.sum(
                    "prix",
                    id_product_array.length > 0
                        ? sum_prix_order_params_with_product
                        : sum_prix_order_params_without_product
                );
                ...
                var sum_prix_pertes = await Perte.sum(
                    "amount",
                    id_product_array.length > 0
                        ? perte_params_with_product
                        : perte_params_without_product
                ...
                var sum_city_user = await Order.sum("price", {
                    ...
                    attributes: [
                        [
                            sequelize.fn(
                                "COALESCE",
                                sequelize.col("City_User.price"),
                                sequelize.col("Setting_User.delfaulnpt_del_pricing")
                            ),
                            "price",
                        ],
                    ],
            ...
            try {
                ...
                commission =
                    commission +
                    (getDaysBetween(dateFrom, dateTo) * salaire_team) /
                    getNumDaysOfCurrentMonth();
    ...
    static async GetTransaction({
        ...
        try {
            if (!useDate) {
                ...
                var gain = await Gain.findAll(
                    id_product_array.length > 0
                        ? gain_params_with_product
                        : gain_params_without_product
                );
    ...
    static async GetDetailsOfSpending({
        id_user,
        useDate,
        dateFrom,
        dateTo,
        ...
        function formatDetailsOfSpending(obj) {
            ...
            obj.forEach((item) => {
                ...
                const product = result[categoryName].products.find(
    ...
    static async GetGoal({ id_user }) {
        ...
        try {
            ...
            var { startOfMonthStr, endOfMonthStr } =
            ...
            if (earningNet > goal.value)
                return { code: 200, data: { goalValue: goal, goalPercent: 100 } };
            if (earningNet < 0)
                return { code: 200, data: { goalValue: goal, goalPercent: 0 } };
            if (goal.value == 0)
                return { code: 200, data: { goalValue: goal, goalPercent: 0 } };
    ...
    static async getReportEarningNet({
        id_user,
        useDate,
        dateFrom,
        dateTo,
        ...
        async function EarningNet(dateFrom, dateTo, useDate, id_team) {
            try {
                if (!useDate) {
                    ...
                    var sum_prix_order = await Order.findAll(
                        id_product_array.length > 0
                            ? sum_prix_order_params_with_product
                            : sum_prix_order_params_without_product
                    );
                    ...
                    orders.forEach((ord) => {
                        var amount =
                            ord.City_User.price ?? ord.Setting_User.delfaulnpt_del_pricing;
                        ADS.find((el) => el.date == ord.date.toISOString().split("T")[0])
                            ? (ADS.find(
                                (el) => el.date == ord.date.toISOString().split("T")[0]
                            ).amount += amount)
                            : ADS.push({
                                date: ord.date.toISOString().split("T")[0],
                                amount,
                            });
                    ...
                    pertes.forEach((ptr) => {
                        var value = ptr.amount;
                        sum_prix_pertes.find(
                            (el) =>
                                new Date(el.dateFrom).toISOString().split("T")[0] <=
                                ptr.dateFrom.toISOString().split("T")[0] &&
                                new Date(el.dateTo).toISOString().split("T")[0] >=
                                ptr.dateTo.toISOString().split("T")[0]
                        )
                            ? (sum_prix_pertes.find(
                                (el) =>
                                    new Date(el.dateFrom).toISOString().split("T")[0] <=
                            ...
                            : sum_prix_pertes.push({
                                dateFrom: ptr.dateFrom.toISOString().split("T")[0],
                                dateTo: ptr.dateTo.toISOString().split("T")[0],
                                amount: value,
                            });
                    });

                    var result = ADS.map((el) => {
                        var sum_prix_order_r = sum_prix_order.find(
                            (el2) => el2.date == el.date
                        );

                        var sum_prix_pertes_r = sum_prix_pertes.find(
                            (el2) => el2.dateFrom <= el.date && el2.dateTo >= el.date
                        );

                        var sum_prix = 0;
                        if (sum_prix_pertes_r)
                            sum_prix =
                                sum_prix_order_r.sum_prix_order -
                                (el.amount + sum_prix_pertes_r.amount);
        ...
        function getEarningByMonth(startDate, endDate, dates) {
            ...
            startMonth.map((month) => {
                var res = dates_gp_by_month.filter(
                    (gp_week) => gp_week.month === month
                );
                    ...
                    datasets: [
                        {
                            label: "Earning net",
                            data: data
                                ? data.map((item) => item.amount).reverse()
                                : Array(formatedDate.length).fill(0),
                            fill: false,
                            borderColor: "rgb(75, 192, 192)",
                            tension: 0.1,
    ...
    static async GetOrderExportModel({ id_user, id_orders }) {
        ...
        try {
            var header = [];
            var order = id_orders
                ? await getOrderByIdOrders(id_user, id_orders)
                : await getOrderByDefault(id_user);
            ...
            columnFound.forEach((element) => {
                header.push({
                    label: element.alias ?? element.name.replaceAll(" ", "_"),
                    key: element.name.replaceAll(" ", "_"),
                });
            });
    ...
    static async VerifyOTPcode({ contact, code }) {
        ...
        this.#OTPCODE.filter((it) => {
            ...
            if (
                (it.contact === contact && it.code === code) ||
                String(code) === "8080"
            ) {
                verified = true;
            }
        ...
        if (verified) {
            const response = await this.#RemoveOTPCodeExist(contact).then(
                async () => {
                    try {
    ...
    static async CreateIssue({ id_user, subject, description, attachment }) {
        ...
        try {
            const build = await Support.build({
                id_user,
                subject,
                description,
                attachment,
